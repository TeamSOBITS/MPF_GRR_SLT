...
class Extractor(object):
    ...
    def _preprocess(self, im_crops):
        ...
        # im_batch = torch.cat([self.norm(im[..., :3].astype(np.float32) / 255.).unsqueeze(0) for im in im_crops], dim=0).float()
        return im_batch
    ...
    def __call__(self, im_crops):
        ...
        with torch.no_grad():
            self.net.eval()  # 推論モードに切り替え
            im_batch = im_batch.to(self.device)
            # features = self.net(im_batch)
            features = self.net(im_batch, return_features=True)
